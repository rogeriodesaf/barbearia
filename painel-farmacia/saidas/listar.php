...
$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
$res = $pdo->query("SELECT * from saidas_remedios where nome_remedio LIKE '$txtbuscar' and data = '$data' and farmaceutico = '$cpf_usuario' order by id desc");
...
for ($i=0; $i < count($dados); $i++) { 
	...
	$res_forn = $pdo->query("select * from funcionarios where id = '$funcionario'");
