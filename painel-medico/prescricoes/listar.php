...
if($txtbuscar == ''){
	$res = $pdo->query("SELECT * from consultas where data = curDate() and medico = '$id_medico' and prescricao = 'sim' order by hora asc LIMIT $limite, $itens_por_pagina");
}else{
	$txtbuscar = @$_POST['txtbuscar'];
	$res = $pdo->query("SELECT * from consultas where data = '$txtbuscar' and medico = '$id_medico' and prescricao = 'sim' order by hora asc");
